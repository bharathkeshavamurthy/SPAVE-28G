...
with ThreadPoolExecutor(max_workers=max_workers) as executor:
    for i in range(len(os.listdir(rx_gps_dir))):
        ...
        try:
            ...
            print('SPAVE-28G | Consolidated Processing MPC | JSONDecodeError caught while parsing {}.'.format(filename))
            continue  # Ignore the JSONDecodeError on this file | Move onto the next file...
        except Exception as e:
            print('SPAVE-28G | Consolidated Processing MPC | Exception caught while parsing {}.'.format(filename))
...
"""
CORE VISUALIZATIONS I: RMS delay spread and RMS direction spread
"""
...
print('SPAVE-28G | Consolidated Processing MPC | RMS Delay Spread CDF: {}.'.format(rms_ds_url))
print('SPAVE-28G | Consolidated Processing MPC | RMS AoA Direction Spread CDF: {}.'.format(rms_aoa_dirs_url))

"""
CORE VISUALIZATIONS II: Cluster inter-arrival times, Cluster decay characteristics, and Cluster peak-power distribution
...
print('SPAVE-28G | Consolidated Processing MPC | Cluster Peak-Power Distribution: {}.'.format(pwrs_url))
print('SPAVE-28G | Consolidated Processing MPC | Cluster Decay Characteristics: {}.'.format(decay_chars_url))
print('SPAVE-28G | Consolidated Processing MPC | Cluster Inter-Arrival Times CDF: {}.'.format(inter_arr_times_url))

"""
CORE VISUALIZATIONS-III: Power Delay Angular Profiles (PDAPs), Power Delay Doppler Profiles (PDDPs), Doppler spectrum
...
print('SPAVE-28G | Consolidated Processing MPC | Doppler Spectrum {}.'.format(doppler_spectrum_url))
print('SPAVE-28G | Consolidated Processing MPC | Power Delay Angular Profiles: {}.'.format(pdaps_url))
print('SPAVE-28G | Consolidated Processing MPC | Power Delay Doppler Profiles: {}.'.format(pddps_url))
