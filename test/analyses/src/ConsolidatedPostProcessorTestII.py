...
# tx_imu_dir, tx_imu_skip_step = 'E:/SPAVE-28G/analyses/urban-campus-I/tx-realm/imu/', 5
...
# mpc_delay_bins = np.arange(start=0, stop=100e-9, step=12.5e-9)
...
# sc_dist_step, sc_align_step, sc_vel_step, mpc_delay_bins = 1.0, 0.05, 1.0, np.arange(start=0, stop=1e-7, step=12.5e-9)
...
def estimate_mpc_parameters(tx: GPSEvent, rx: GPSEvent, n: int, x: np.array) -> List:
    # Additional windowing for computational feasibility
    x_sage = x[int(0.5 * w_sage):int(1.5 * w_sage)] if n > 2 * w_sage else x
    n_sage = x_sage.shape[0]

    f_c, _f_c = tx_fc, rx_fc
    f_s, n_std = sample_rate, n_sigma
    first_iter, v_0, l_, k_ = False, pn_v0, pn_l, pn_reps

    y_f = np.fft.fft(x_sage) / n_sage
    fs = np.argsort(np.fft.fftfreq(n_sage, (1 / f_s)))
    ...
    # Convergence check
    def is_converged(iter_idx, l_idx) -> bool:
        ...
        if first_iter or \
            ...
            return False if iter_idx <= iter_max else True

        return True

    # SAGE wrapper
    for l_mpc in range(max_mpcs):
        iter_num, first_iter = 0, True

        print(f'SPAVE-28G | Consolidated Processing II | Determining the parameters associated with MPC: {l_mpc}!')

        while not is_converged(iter_num, l_mpc):
            first_iter = False
            iter_num += 1
        ...
        print(f'SPAVE-28G | Consolidated Processing II | Finished det. the parameters associated with MPC: {l_mpc}!')
...
print('SPAVE-28G | Consolidated Processing II | Reading power delay profiles and their associated metadata!')
...
print('SPAVE-28G | Consolidated Processing II | Finished reading power delay profiles and their associated metadata!')

''' Match gps_event, imu_trace, and pdp_segment timestamps across both the Tx and the Rx realms '''

seqnums = np.random.choice(len(rx_gps_events), eval_cnt)

for seqnum in seqnums:
    print(f'SPAVE-28G | Consolidated Processing II | Started processing event-trace-measurement sequence: {seqnum}!')
    ...
    print('SPAVE-28G | Consolidated Processing II | Estimating multipath components and their associated parameters!')

    mpc_parameters = estimate_mpc_parameters(tx_gps_event, rx_gps_event,
                                             pdp_segment.num_samples, pdp_segment.processed_rx_samples)

    print('SPAVE-28G | Consolidated Processing II | Finished est. multipath compnts and their associated parameters!')
    ...
    pods.append(Pod(seq_number=seq_number, timestamp=timestamp,
                    tx_gps_event=tx_gps_event, tx_imu_trace=IMUTrace(),
                    rx_gps_event=rx_gps_event, rx_imu_trace=IMUTrace(),
                    mpc_parameters=mpc_parameters, pdp_segment=pdp_segment))
...
"""
CORE VISUALIZATIONS II: RMS delay spread and RMS direction spread (component based...)
"""
