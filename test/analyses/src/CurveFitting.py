...
def linear_fit(filename: str, sheetname: str = 'Pathlosses',
    ...
    # print(f'x_min: {np.min(x)} | x_max: {np.max(x)} | Fit Parameters: {np.polyfit(np.log10(x), y, deg=1)}')
    return np.min(x), np.max(x), np.polyfit(np.log10(x), y, deg=1)


def exponential_fit(filename: str, sheetname: str = 'Spatial_Correlation_Coefficient', xlengine: str = 'openpyxl',
                    xcolname: str = 'Distance', ycolname: str = 'Spatial Autocorrelation Coefficient') -> Tuple:
    """
    Examples:
